...
https://github.com/projectriff/riff[Riff] is a container runtime with strong links to `pack` and `kpack` for building images. There is a CLI to download, and a https://projectriff.io/docs/v0.4/getting-started/minikube[Getting Started Guide] (the Minikube version works with `kind` if you start from the section entitled https://projectriff.io/docs/v0.4/getting-started/minikube#install-helm[Install Helm]). Install the riff system in the cluster:
...
NOTE: like with `pack` you can either build from source or from the executable jar file.
...
So we can connect to it using a forward (for instance):
...
To update the application we delete it and recreate:
...
The Riff CLI is just a convenience wrapper around a container build, plus a few lines of YAML. If you already bbuilt the container a different way, like with a Dockerfile, you can create the YAML manually and simply apply it with `kubcetl`. The two `riff` invocations result in 2 API objects:
...
If you are running a MySQL service already on the cluster you can bind to it using the `--env` and `--envFrom` options on the `riff core deployer create` command. Or you can go the YAML route and bind to the configuration in the deployer spec. Example:
...
Combine that with a config map called "env-config" that was created by your MySQL service, and you might have a functional Pet Clinic.

The deployer spec is just a pod spec, so you can add other things as well, like volume mounts. If you have an `application.properties` file created by the MySQL service manifest in a config map called "mysql-config", then this might be a good way to read it into the Spring Boot application. Here's a `kustomize` patch for the deployer:
...
`app.yaml` is the application and deployer declarations, and `binding.yaml` is the patch above.
